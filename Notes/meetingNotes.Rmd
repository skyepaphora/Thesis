...
#### Glen Talk
- Null hyp: no modulated signal where (modulation is degree p) at a center frequency
- start with a center frequency, design a modulating function
- in our lines, it should give back the freq at the center time
- Glen's happy with limiting scope to linear/quadratic
- BRO YOU GOTTA USE EQ 2.32
- yo $m_p$ is the modulation bandwidth. Ben's is usually small, like 0.01
  mod bandwitdth shouldn't be bigger than W!!

Premise:
We can use spectrograms as a visual diagnostic for what kind of modulating function to look for
^ no! other way around. Ben/Kian tell us there's modulation, we use my techniques to estimate

**Main necessary goal for thesis:**
- Estimating mod func and spec of a k-gump.
- confirming limitations (identifyability)
- YES; we can estimate the outer product of these vecs, smoothed.
- NO; scaling issues? For the smooth pbc? certainly not for k>1

- Good enough: estimates of each UMP component, *weighting of each*
                and if we can't, we just need a good reason as to why 
                [like for k=1 there was the A = BC issue]

Glen's idea for GUMP decomp into UMP comps
sets of columns. 
each column is a lin mod of 2 mod funcs.
BSS estimates these funcs.
Do this for many sets of columns. 
Then do it for many sets of spectra.
but BSS doesn't distinguish between c1 and c2 each time.

  So just mix and match,
  what is the line combo that has the least distance (euclidean, least squares, etc.)
  from the initial estimator (pbc, hrs)
  
  Or look at the eigenvalues/vecs of the pbc matrix
  and get the lin combo with the closest eigenvalues
  
And then suggest how this would generalized to k-GUMP

## Thursday

0.  just fucking figure out the freq bandwidth for the sliding window
1.  Look at (2.32) in Ben's thesis and implement that kind of mod func, with modband < windowband
*2.  Start building a narrative with Kian/Ben --> Skye*
    *Set up various polymods to be selected at random, identified* (maybe not today?)
  

**Return to GUMP decomp**
1.  Why are we getting scaling issues for the smoothed pbc?
2.  Look into the component weighting method described, above (Glen)
    Revive old code (Jul/Aug) if necessary
3.  Also revive old split fix code, start writing draft of this in doc

**Then it's OsTime**

#### Let's go:

0.  NW = 4 by default --> W = 4/B. 
    I tried changing this but obviously you get bigger lobes for NW < 4
1.

$$
X_t = \sum_{m=0}^{M-1} \mu_m \cos\left(2\pi f_mt + 2\pi \int_{0}^t\phi_m(\tau)d\tau\right) + Z_t
$$

Call the integral $\psi_m(t)$

* I tried both a quadratic and a linear modulating function.
  In ben's thesis, he claims the $\phi_m$ should stay within the band (-W,W).
  For some reason, the linear modulation fits perfectly within the window W band?
    Maybe a coincidence, note that I did base $\phi_m$'s range on the W computed from N
* The quadratic I used was wack, I'll investigate quads once we talk about linear a bit more

**Back to PBC**
1.  I played around and was eventually able to get the smoothed estimate's range to match the pBC.
...
# Oct 31 
1. look at Ben's thesis, take examples from there
  code is in the ben files
  read thesis for interpretation of this integral thing


# Nov 7 
Skye is sad
fit a plane constrained to the point of interest: (t,f) - you suspect a freq f at some time t
then fit a regression 

do the HRS -- does it still do the doubling thing?

find a plane which divides the 
SVMs - support vector machine
go across each freq at t=500
look for the freq where the plateau begins
fit lines across all those plateau pieces
then take the average of all of those slopes

okay the pbc might help distinguish splitting from nearby mod-sigs in ben's test,
but does our W_B fuck it up?

Q: can we extend this to larger bandwidths than Ben?

# Nov 11

1. check that the BSS output components have scalings that work
2. Check that SVD matches the right g to the right s (g1-s1 and g2-s2)
    - 1st left eigen = corresponds to larges "singular value" (total power of first component)
    - same is true for right eigens.
    --> 1st and 1st belong together because they apply to the same singular value
    --> the singular values may give insight into weights of the components
    singluar1*(leigen1 + reigen1) + singluar2*(leigen2 + reigen2)
  Take pbc -> compute svd -> see what these values are -> compare first to tru
  -> Then use those to select which component is which
  -> and even: which g looks more like the kth left eign, which s looks more like the kth right eign
  -> reminder: singular values might give us weights

#### For meeting

* note that the way I'm constructing y means there's always noise, right? 
* BSS: I was also cheating. I swept using the true c, which we won't know.
* cool result: 
  large B gives smoother gs with worse edges, worse Ss
  small B gives smoother Ss with worse gs, and Ss are wobbly
* For some reason, mine keeps giving the negative comps of UMP1 for UMP2
  -- worse when more pairs are bss-timated
  -- I made option to correct for this in bss.qmd: used modulo to restrict, 
      get different results if you swap g1/g2, etc. in the sweep 
      -- Okay this does help for the ception I guess
* plugged SVD estimates into BSS so it wouldn't be cheating, messy
* out of curiosity I tried plugging the bss estimates back into the bss,
  basically just smooths, also somehow 1 and 2 start to resemble eachother even when I toggle modulo
QUESTION: why does BSS-ception just turn into the 1st component? *EVEN* when I do the swap at #**
OKAY next actual thing: apply over a bunch of simulations to get mean and CIs



# Nov 21

* take (good) estimates of g1, S1, take outer product, then scale
* subtract it from the pbc, what's left over should be a pbc of UMP2
* consider smoothing the output of bss, under the assumption g is smooth
* consider trying other BSS functions (other than JADE?)

* *another option:* rather than comparing to some input (cheat, svd),
  we use clustering. We group the 4 vecs into what are similar to each other over the pairs.
  The most important thing is making sure the right g's go with the right s.
  *also* we could optimize a linear combo of the outer products of different combos of these estimates

- maybe we can smooth g1 and leverage the shape to improve s1 somehow?
- the failiure of the SVD is still worth discussing in thesis, demonstrates difficulty of the problem

#### Task for next time
* do the 4-way clustering. take average within each cluster. plot them.
  Do any of them give a good estimate of g1, s1?
  - Glen recommends staying with the current g/s functions (up-line, norm, Ar2, Ar1)
